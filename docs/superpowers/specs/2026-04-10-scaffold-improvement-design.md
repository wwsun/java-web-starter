...
- 权限：`user:read`、`user:write`、`user:delete`、`role:read`、`role:write`
- `ADMIN` 拥有全部权限，`USER` 仅拥有 `user:read`
- 默认 `admin` 账号分配 `ADMIN` 角色

### 后端实现

**新增模块** `module/role/`（结构与 `module/user/` 一致）：

```
module/role/
├── controller/RoleController.java        # GET /roles, POST /users/{id}/roles
├── controller/PermissionController.java  # GET /permissions（只读）
...
**安全层变更**：
- `CustomUserDetailsService`：加载用户时联表查询角色和权限，填入 `SimpleGrantedAuthority`
- `SecurityConfig`：添加 `@EnableMethodSecurity` 注解
- `UserController.deleteUser`：添加 `@PreAuthorize("hasAuthority('user:delete')")` 作为示范
- `UserVO`：新增 `roles: List<String>` 字段（角色 code 列表）

**不引入**：自定义 `@RequireRole` 注解，直接使用 Spring Security 原生 `@PreAuthorize`，更标准。
